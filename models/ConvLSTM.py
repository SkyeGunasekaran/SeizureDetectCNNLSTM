...
def calculate_same_padding(input_size, kernel_size, stride):
    ...
    pad = max(((output_size - 1) * stride + kernel_size - input_size) / 2, 0)
    return int(pad)
...
class ConvNN(nn.Module):
    def __init__(self, X_train_shape, nb_classes = 2):
        ...
        self.conv2 = nn.Conv2d(16, 32, kernel_size=(3, 3), padding=calculate_same_padding(self.fs, 3, 1))
        self.pool2 = nn.MaxPool2d(kernel_size=(2, 2))

        self.normal3 = nn.BatchNorm2d(32)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=(3, 3), padding=calculate_same_padding(self.fs, 3, 1))
