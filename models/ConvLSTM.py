...
class ConvNN(nn.Module):
    def __init__(self, X_train_shape, nb_classes=2):
        ...
        self.conv1 = nn.Conv3d(in_channels=1, out_channels=16, 
                               kernel_size=(X_train_shape[2], 5, 5), stride=(1, 2, 2), padding=(0, 2, 2))
    ...
    def fit(self,
        ...
        if os.path.exists(f"weights_{target}_{mode}.pth"):
            self.load_state_dict(torch.load(f"weights_{target}_{mode}.pth", map_location='cuda'))
            if mode == 'cv':
